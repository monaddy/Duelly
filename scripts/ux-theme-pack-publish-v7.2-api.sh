#!/usr/bin/env bash
# UX/DesignOps — Theme Pack Publisher v7.2 (GitHub API, debug+fix for /git/refs/*)
set -Eeuo pipefail
...
trap 'c=$?; echo "::DUELLY::step='${STEP_ID}' status=error time=$(date -Is) code=$c log='${LOG_FILE}' notes=\"theme pack publish failed (v7.2)\""; exit $c' ERR
...
h() { echo "  > $*"; }
jget() { node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{try{let j=JSON.parse(s);let p='${1:-}'.split('.');for(let k of p){if(!k)continue;j=j[k]}console.log(j??'')}catch{}})"; }

# ==== 1) Build theme.css (Tailwind → fallback) ====
...
if [ $RC -eq 0 ] && [ -s "$OUT_PATH" ] && grep -q '\.bg-accent' "$OUT_PATH" && grep -q '\.text-fg' "$OUT_PATH" && grep -Eq '\.bg-surface(\\\/|/)60' "$OUT_PATH"; then
  TAIL_OK="yes"; h "[OK] tailwind build → $OUT_PATH"
else
  h "[FALLBACK] generating minimal CSS"
  cat > "$OUT_PATH" <<'CSS'
...
REPO_JSON="$(curl -sS -H "Authorization: token $TOKEN" -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: $API_VER" "$API/repos/$OWNER/$REPO")"
DEFAULT_BRANCH="$(printf '%s' "$REPO_JSON" | jget 'default_branch')"; [ -n "$DEFAULT_BRANCH" ] || DEFAULT_BRANCH="main"
h "repo=$OWNER/$REPO default=$DEFAULT_BRANCH"

# helper: GET with http code + echo snippet on failure
...
# ==== 3) Publish flow (target → staging → fallback) ====
publish_branch() {
  local BR="$1"; h "[PUBLISH] branch=$BR"
  # does branch exist?
  local CODE; CODE=$(get_api "$API/repos/$OWNER/$REPO/git/refs/heads/$BR" "$TMP_DIR/refs-$BR")
  if [ "$CODE" = "404" ]; then
    # create branch from default
    local CODE2; CODE2=$(get_api "$API/repos/$OWNER/$REPO/git/refs/heads/$DEFAULT_BRANCH" "$TMP_DIR/refs-def-$BR")
    local BASE_SHA="$(cat "$TMP_DIR/refs-def-$BR.b" | jget 'object.sha')"
    if [ -z "$BASE_SHA" ]; then
      # fallback via /branches API
      CODE2=$(get_api "$API/repos/$OWNER/$REPO/branches/$DEFAULT_BRANCH" "$TMP_DIR/branches-def-$BR")
      BASE_SHA="$(cat "$TMP_DIR/branches-def-$BR.b" | jget 'commit.sha')"
    fi
    [ -n "$BASE_SHA" ] || { echo "[ERR] cannot resolve base SHA for $DEFAULT_BRANCH"; return 3; }
    local BODY="{\"ref\":\"refs/heads/$BR\",\"sha\":\"$BASE_SHA\"}"
    local C3; C3=$(post_api "$API/repos/$OWNER/$REPO/git/refs" "$TMP_DIR/create-$BR" "$BODY")
    [ "$C3" = "201" ] || { echo "[ERR] create ref failed ($C3):"; tail -n 5 "$TMP_DIR/create-$BR.b" || true; return 4; }
  elif [ "$CODE" != "200" ]; then
    echo "[ERR] refs check failed ($CODE):"; tail -n 5 "$TMP_DIR/refs-$BR.b" || true; return 2
  fi
  ...
  if [ "$C5" = "201" ] || [ "$C5" = "200" ]; then
    echo "$BR"; return 0
  fi
  echo "[ERR] put file failed ($C5):"; tail -n 10 "$TMP_DIR/put-$BR.b" || true; return 5
}
...
[ -n "$BR_DONE" ] || { echo "::DUELLY::step=$STEP_ID status=error time=$(date -Is) code=PUBLISH_FAILED log=$LOG_FILE notes=\"API publish failed to all branches\""; exit 1; }
