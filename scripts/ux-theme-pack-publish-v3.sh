#!/usr/bin/env bash
# UX/DesignOps — Theme Pack Publisher v3 (A1/T1)
# ניסיון Tailwind-CLI -> פלייבק ידני, git add -f, push לענף ops/assets, חתימת ChatOps.
...
trap 'c=$?; echo "::DUELLY::step='${STEP_ID}' status=error time=$(date -Is) code=$c log='${LOG_FILE}' notes=\"theme pack publish failed (v3)\""; exit $c' ERR
...
# 1) קונפיג Tailwind מינימלי + safelist
cat > "$ASSET_CFG" <<'CFG'
module.exports = {
...
# 2) ניסיון יצירה עם Tailwind CLI (עם התקנה מקומית במידת הצורך)
TAIL_OK="no"
set +e
...
# 3) פלייבק ידני אם CLI נכשל/לא יצר את המחלקות הנדרשות
REQ_OK="no"
if [ "$TAIL_OK" = "yes" ]; then
  if grep -q '\.bg-accent' "$OUT" && grep -q '\.text-fg' "$OUT" && grep -Eq '\.bg-surface(\\\/|/)60' "$OUT"; then
    REQ_OK="yes"
  fi
fi
...
# 5) בדיקת AA (text-fg על bg-accent)
node - <<'JS'
...
function contrast(h1,h2){const A=hexToRgb(h1),B=hexToRgb(h2); const L1=relLum(A),L2=relLum(B); const hi=Math.max(L1,L2), lo=Math.min(L1,L2); return (hi+0.05)/(lo+0.05);}
const ratio=contrast("#0F172A","#10B3B3"); if (ratio<4.5){ console.error("[ERR] contrast",ratio.toFixed(2)); process.exit(22);} else { console.log("[AA] contrast",ratio.toFixed(2)); }
JS

# 6) commit + push (לעקוף .gitignore)
