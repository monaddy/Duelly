...
function insertIntoExtendColors(src) {
  ...
  if (colorsMatch) {
    // Insert inside existing colors block
...
// Fallback: append a CommonJS self-healing patch if module.exports present
if ((!out || out === s) && s.includes('module.exports')) {
  out = s + "\n/* DUELLY-AUTO-ACCENT-FALLBACK */\n;(() => { try { const cfg = module.exports||{}; const t=cfg.theme=cfg.theme||{}; const e=t.extend=t.extend||{}; const c=e.colors=e.colors||{}; if(!('accent' in c)) c.accent={ DEFAULT: '#10B3B3' }; module.exports=cfg; } catch(e){} })();\n";
}

if (out && out !== s) {
  fs.writeFileSync(path, out, 'utf8');
  console.log('accent inserted');
  process.exit(0);
} else {
  console.log('no change performed (possibly already present or non-standard config)');
  process.exit(0);
}
