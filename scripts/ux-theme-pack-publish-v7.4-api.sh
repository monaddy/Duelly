...
trap 'c=$?; echo "::DUELLY::step='${STEP_ID}' status=error time=$(date -Is) code=$c log='${LOG_FILE}' notes=\"theme pack publish failed (v7.4)\""; exit $c' ERR
...
if [ -z "$TOKEN" ] || [[ "$TOKEN" == *"<"*">"* ]]; then
  echo "::DUELLY::step=$STEP_ID status=error time=$(date -Is) code=MISSING_PAT log=$LOG_FILE notes=\"Set GH_PAT (no <>) with 'repo' scope\""; exit 1;
fi

API="https://api.github.com"; API_VER="2022-11-28"

# --- נירמול slug של הריפו (מסיר .git, שולי / ו-CR/LF) + אפשרות Override ידני ---
clean_slug() {
  printf '%s' "$1" | tr -d '\r' | \
    sed -E 's#(git@|https://)github.com[:/]+##; s#^/+##; s#/+$##; s#\.git$##'
}
if [ -n "${REPO_PATH_OVERRIDE:-}" ]; then
  REPO_PATH="$(clean_slug "$REPO_PATH_OVERRIDE")"
else
  REPO_PATH="$(clean_slug "$ORIGIN_URL")"
fi
...
if [ -z "$OWNER" ] || [ -z "$REPO" ]; then
  echo "::DUELLY::step=$STEP_ID status=error time=$(date -Is) code=PARSE_REPO log=$LOG_FILE notes=\"Failed to parse owner/repo from origin. Set OWNER_OVERRIDE/REPO_OVERRIDE.\"" ; exit 1
fi
...
if [ "$CODE" != "200" ]; then
  echo "[ERR] /repos failed (http=$CODE)"; tail -n 8 "$TMP_DIR/repo.b" || true
  echo "::DUELLY::step=$STEP_ID status=error time=$(date -Is) code=REPO_ACCESS log=$LOG_FILE notes=\"GitHub API /repos failed (http=$CODE). Check token access or OWNER/REPO.\"" ; exit 1
fi
...
publish_branch() {
  ...
  if [ "$CODE" = "404" ]; then
    ...
    [ "$C3" = "201" ] || { echo "[ERR] create ref failed (http=$C3)"; tail -n 10 "$TMP_DIR/create-$BR.b" || true; return 4; }
  elif [ "$CODE" != "200" ]; then
  ...
  local BODY="{\"message\":\"$COMMIT_MSG\",\"content\":\"$CONTENT_B64\",\"branch\":\"$BR\""; [ -n "$FILE_SHA" ] && BODY="$BODY,\"sha\":\"$FILE_SHA\""; BODY="$BODY}"
