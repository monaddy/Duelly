...
export async function hmacSha256Hex(keyHex: string, data: Uint8Array): Promise<string> {
  ...
  const sig = (() => { const __d = (data); const __view = (__d instanceof Uint8Array) ? __d : new Uint8Array((__d && __d.buffer) ? __d.buffer.slice(0) : __d); return (() => { const __d = (__view); const __u8 = (__d instanceof Uint8Array) ? __d : new Uint8Array(ArrayBuffer.isView(__d) ? __d.buffer.slice(0) : (__d || new ArrayBuffer(0))); return crypto.subtle.sign('HMAC', key, __u8); })(); })();
