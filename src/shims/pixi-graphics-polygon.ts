...
declare module 'pixi.js' { interface Graphics { polygon(points: number[] | IPointData[], close?: boolean): this; } }
(function attachPolygon() {
  // @ts-expect-error runtime augment
  ...
  proto.polygon = function(points: number[] | IPointData[], close: boolean = true) {
    ...
    if (arr.length) {
      ...
      else { for (let i=0;i<arr.length;i++) { const p=arr[i] as IPointData; pushPoint(p.x,p.y,i); } }
    }
